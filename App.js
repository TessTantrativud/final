...
export default class App extends React.Component {
  ...
  getIdeas() {
    ...
    let airtableUrl = "https://api.airtable.com/v0/appMY7PKpYgX60sSo/foodbucket?&view=Grid%20view";

    // Needed for Airtable authorization, replace with your own API key
    let requestOptions = {
      headers: new Headers({
        'Authorization': 'Bearer keyXaDid6FG1zrqKM'
      })
    };
  ...
  upvoteIdea(data, secId, rowId, rowMap) {
    ...
    let airtableUrl = "https://api.airtable.com/v0/appMY7PKpYgX60sSo/foodbucket/" + data.id;

    // Needed for Airtable authorization
    let requestOptions = {
      method: 'PATCH',
      headers: new Headers({
        'Authorization': 'Bearer keyXaDid6FG1zrqKM', // replace with your own API key
  ...
  render() {
    ...
    return (
      <Container>
        <Header>
          <Body>
            <Title>Chicago Food Review List</Title>
          </Body>
        </Header>
        <Content>
          <List
